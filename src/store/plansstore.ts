  ...
  fetchPlans: async () => {
    ...
    const formattedPlans: Plan[] = data.map((p) => {
      ...
      return {
        ...
        color: p.color,
        trialDays: (p as any).trial_days ?? 14,
        allowCoupons: (p as any).allow_coupons ?? true,
        usageLimit: (p as any).usage_limit ?? null,
        allowedPages: (p as any).allowed_pages ?? null
      };
    });
    ...
    const { error } = await supabase.functions.invoke('admin-stripe-plan', {
      body: {
        ...
        plan: {
          ...
          allowCoupons: (plan as any).allowCoupons,
          usageLimit: (plan as any).usageLimit,
          allowedPages: (plan as any).allowedPages,
        },
      },
    });
