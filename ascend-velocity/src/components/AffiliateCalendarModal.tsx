...
export function AffiliateCalendarModal({ isOpen, onClose, affiliate, calendarStatuses, classes }) {
  ...
  useEffect(() => {
    ...
    function createMonthTable(data: any, option: any) {
      ...
      while (c <= 6) {
        ...
        applyStatusStyle(td, count, data.monthIndex, data.year, option, data);
        tr.appendChild(td);
        count++;
        c++;
      }
    ...
    function applyStatusStyle(td: HTMLElement, day: number, month: number, year: number, option: any, data: any) {
       if (option.highlighttoday && data.today.date === day && data.today.monthIndex === data.monthIndex) {
          td.classList.add("dycalendar-today-date");
       }
       if (option.highlighttargetdate && option.date === day && data.monthIndex === option.month) {
          td.classList.add("dycalendar-target-date");
       }

       if (affiliate && calendarStatuses && classes) {
           const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
           const key = `${affiliate.id}:${dateStr}`;
           const statusKey = calendarStatuses[key];
           
           if (statusKey) {
               const cls = classes.find((c: any) => c.key === statusKey);
               if (cls) {
                   td.style.background = cls.bg || '#ccc';
                   td.style.color = cls.textColor || '#fff';
                   td.title = `${cls.label} (${cls.points} pts)`;
               }
           }
       }
    }
    ...
    onClick();
    drawCalendar(opts);
  }, [isOpen, calendarStatuses, classes, opts]); // Added dependencies
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-white/90 text-sm">
                 <span>{affiliate?.points || 0} pts</span>
                 <span>•</span>
                 <span>{affiliate?.level || 'Iniciante'}</span>
              </div>
            </div>
          </div>
