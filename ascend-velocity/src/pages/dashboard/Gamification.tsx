...
import { Trophy, Award, Zap, Plus, Check, X, SquarePen, Trash2, Users, Lock, FileText, Lightbulb } from "lucide-react";
...
export default function Gamification() {
  ...
  const { startGamificationTutorial } = useTutorial();
  const [form, setForm] = useState({ label: "", description: "", points: 0, bg: "#ffffff", type: "neutral" as "positive" | "negative" | "neutral" });
          ...
          <GlassCard className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <div className="text-sm text-muted-foreground">Faça login para continuar.</div>
              </div>
            </div>
          </GlassCard>
        </div>
  ...
  return (
    <DashboardLayout>
      <div className="p-4 md:p-6 lg:p-8">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8" id="gamification-header">
          <div>
          ...
          <Button
            onClick={startGamificationTutorial}
            variant="outline"
            className="flex gap-2 bg-white/10 hover:bg-white/20 border-white/20 text-foreground px-6 self-start md:self-auto"
          >
        ...
        {/* Affiliate Progress Section Removed */}

        {/* Levels Section */}
        <div className="mb-6 md:mb-8" id="gamification-levels">
        ...
        <div className="mb-8" id="gamification-achievements">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold flex items-center gap-2">
              <Award className="w-6 h-6 text-neon-violet" />
              Conquistas
            </h2>
            <NeonButton id="gamification-add-achievement" variant="glass" className="flex items-center gap-2" onClick={() => setEditingAchievement({ 
              id: "", 
              title: "", 
              description: "", 
              xp: 0, 
          ...
          <Dialog open={!!editingAchievement} onOpenChange={(open) => !open && setEditingAchievement(null)}>
            <DialogContent className="w-[95vw] sm:w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-popover/80 backdrop-blur-xl border-border text-popover-foreground p-4 md:p-6 rounded-xl">
              <DialogHeader>
                <DialogTitle>{editingAchievement?.id ? "Editar Conquista" : "Nova Conquista"}</DialogTitle>
              </DialogHeader>
                ...
                <div className="grid gap-4 py-4">
                  <div className="flex gap-4 mb-2">
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          type="number"
                          value={isNaN(editingAchievement.xp) ? "" : editingAchievement.xp}
                          onChange={(e) => setEditingAchievement({ ...editingAchievement, xp: e.target.value === "" ? NaN : Number(e.target.value) })}
                          className="bg-background/50 backdrop-blur-sm px-3 py-2 rounded-xl border border-input w-full"
                        />
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        type="number"
                        value={editingAchievement.streakDays === undefined || isNaN(editingAchievement.streakDays) ? "" : editingAchievement.streakDays}
                        onChange={(e) => setEditingAchievement({ ...editingAchievement, streakDays: e.target.value === "" ? NaN : Number(e.target.value) })}
                        className="bg-background/50 backdrop-blur-sm px-3 py-2 rounded-xl border border-input w-full"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mb-2">Selecione para quais classes essa conquista é válida.</div>
                    <div className="flex flex-wrap gap-2">
                      {classes.map((c) => {
              ...
              <DialogFooter>
                  ...
                  onClick={() => {
                    if (editingAchievement) {
                      if (editingAchievement.id) {
                        ...
                        const base = editingAchievement.title
                          .toLowerCase()
                          .trim()
                          .normalize("NFD")
                          .replace(/[\u0300-\u036f]/g, "")
                        ...
                        const id = `${base || "conquista"}_${suffix}`;
                        addAchievement({ ...editingAchievement, id });
                ...
                >
                  {editingAchievement?.id ? "Salvar Alterações" : "Criar Conquista"}
                </NeonButton>
              </DialogFooter>
            </DialogContent>
          </Dialog>
              ...
              {editingLevel && (
                <div className="grid gap-4 py-4">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={isNaN(editingLevel.minXP) ? "" : editingLevel.minXP}
                      onChange={(e) => setEditingLevel({ ...editingLevel, minXP: e.target.value === "" ? NaN : Number(e.target.value) })}
                      placeholder="Ex: 0"
                      className="bg-background/50 backdrop-blur-sm px-3 py-2 rounded-xl border border-input w-full"
                    />
                  </div>
          ...
          <Dialog open={isAddingClass} onOpenChange={setIsAddingClass}>
            <DialogContent className="w-[95vw] sm:w-full max-w-lg max-h-[90vh] overflow-y-auto bg-background/95 backdrop-blur-2xl border-none shadow-2xl rounded-3xl p-0">
              <DialogTitle className="sr-only">Nova Classe</DialogTitle>
              <div className="relative h-32 bg-gradient-to-br from-primary/20 via-secondary/20 to-accent/20">
                ...
                <div className="absolute bottom-6 left-8">
                  <h2 className="text-2xl font-bold text-foreground">Nova Classe</h2>
                  <p className="text-sm text-muted-foreground">Adicione uma nova classe de pontuação</p>
                </div>
              </div>
              ...
              <div className="p-8 space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="grid gap-2">
                      ...
                      <div className="relative group">
                        <input
                          type="number"
                          value={isNaN(form.points) ? "" : form.points}
                          onChange={(e) => setForm({ ...form, points: e.target.value === "" ? NaN : Number(e.target.value) })}
                  ...
                  <div className="grid gap-2">
                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Tipo de Dia</label>
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        { value: 'positive', label: 'Positivo', color: 'bg-green-500/10 text-green-500 border-green-500/20' },
                        { value: 'negative', label: 'Negativo', color: 'bg-red-500/10 text-red-500 border-red-500/20' },
                        { value: 'neutral', label: 'Neutro', color: 'bg-gray-500/10 text-gray-500 border-gray-500/20' }
                      ].map((type) => (
                        <button
                          key={type.value}
                          onClick={() => setForm({ ...form, type: type.value as any })}
                          className={`px-3 py-2 rounded-xl text-sm font-medium border-2 transition-all duration-300 ${
                            form.type === type.value 
                              ? type.color.replace('border-', 'border-') + ' ring-2 ring-offset-1 ring-offset-background'
                              : 'bg-gray-50 dark:bg-white/5 border-transparent text-muted-foreground hover:bg-gray-100 dark:hover:bg-white/10'
                          }`}
                        >
                          {type.label}
                        </button>
                ...
                <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-white/5">
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setForm({ label: "", description: "", points: 0, bg: "#ffffff", type: "neutral" });
          ...
          <Dialog open={!!editingClass} onOpenChange={(open) => !open && setEditingClass(null)}>
            <DialogContent className="w-[95vw] sm:w-full max-w-lg max-h-[90vh] overflow-y-auto bg-background/95 backdrop-blur-2xl border-none shadow-2xl rounded-3xl p-0">
              <DialogTitle className="sr-only">Editar Classe</DialogTitle>
              <div className="relative h-32 bg-gradient-to-br from-primary/20 via-secondary/20 to-accent/20">
                <div className="absolute inset-0 bg-white/40 dark:bg-black/40 backdrop-blur-[2px]" />
                <div className="absolute bottom-6 left-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div className="grid gap-2">
                        ...
                        <div className="relative group">
                          <input
                            type="number"
                            value={isNaN(editingClass.points) ? "" : editingClass.points}
                            onChange={(e) => setEditingClass({ ...editingClass, points: e.target.value === "" ? NaN : Number(e.target.value) })}
                            className="w-full bg-gray-50 dark:bg-white/5 border-2 border-transparent focus:border-primary/50 rounded-2xl px-4 py-3 outline-none transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/10 font-mono font-bold text-lg"
                          />
                          <div className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground/50">
                            <span className="text-xs font-bold">PTS</span>
                    ...
                    <div className="grid gap-2">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          <button
                            key={type.value}
                            onClick={() => setEditingClass({ ...editingClass, type: type.value as any })}
                            className={`px-3 py-2 rounded-xl text-sm font-medium border-2 transition-all duration-300 ${
                              (editingClass.type || 'neutral') === type.value 
                                ? type.color.replace('border-', 'border-') + ' ring-2 ring-offset-1 ring-offset-background'
                                : 'bg-gray-50 dark:bg-white/5 border-transparent text-muted-foreground hover:bg-gray-100 dark:hover:bg-white/10'
                            }`}
                          >
