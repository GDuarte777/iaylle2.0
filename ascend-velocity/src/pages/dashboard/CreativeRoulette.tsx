...
function CreativeRouletteInner() {
  const { user } = useAuthStore();
  const access = useAccessControl();
  const { startRouletteTutorial } = useTutorial();
    ...
    const loadChallenges = async () => {
      try {
        ...
        if (data) {
          ...
          await supabase.from('user_roulette_challenges').insert({
            user_id: user.id,
  ...
  const updateChallenges = async (newChallenges: Challenge[]) => {
    if (!ensureToolsEnabled()) return false;
  ...
  const addChallenge = async () => {
    ...
    const ok = await updateChallenges([...challenges, newChallenge]);
    if (!ok) return;
  ...
  const updateChallenge = async () => {
    ...
    const ok = await updateChallenges(challenges.map(c => 
  ...
  const removeChallenge = async (id: string) => {
    ...
    const ok = await updateChallenges(challenges.filter(c => c.id !== id));
    if (!ok) return;
    ...
    <div className="container mx-auto p-4 md:p-8 max-w-6xl">
        <header className="mb-8 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-2">
            <Button
              onClick={startRouletteTutorial}
              variant="outline"
              className="border-neon-blue/20 hover:bg-neon-blue/10 flex gap-2"
            >
              <Lightbulb className="w-4 h-4 text-yellow-500" />
              Como usar
            </Button>
            <Button 
              id="roulette-manage-btn"
              variant="outline" 
              className="border-neon-blue/20 hover:bg-neon-blue/10"
              onClick={() => {
                if (!ensureToolsEnabled()) return;
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="flex flex-col items-center justify-center">
            <div className="relative w-[300px] h-[300px] md:w-[400px] md:h-[400px]" id="roulette-wheel">
              {/* Pointer */}
                      ...
                      return (
                        <g key={challenge.id}>
                          ...
                          <text
                            ...
                            transform={`rotate(${(index * 360 / challenges.length) + (180 / challenges.length)}, ${(startX + endX) / 3}, ${(startY + endY) / 3})`}
            ...
            <div className="mt-8 w-full max-w-xs" id="roulette-spin-btn">
              <motion.button
                onClick={spin}
                disabled={isSpinning || challenges.length === 0 || (access ? !access.isPageToolsEnabled() : false)}
                ...
                className={`
                  ...
                  ${isSpinning || (access ? !access.isPageToolsEnabled() : false)
          ...
          <div className="flex flex-col gap-6">
             <GlassCard className="p-6 min-h-[200px] flex flex-col justify-center relative overflow-hidden border-neon-blue/20 dark:border-neon-blue/20 border-neon-blue/10 bg-white/50 dark:bg-black/50" id="roulette-result-card">
        ...
        <Dialog
          open={isManageOpen}
          onOpenChange={(open) => {
            if (open && !ensureToolsEnabled()) return;
            setIsManageOpen(open);
          }}
        >
          <DialogContent className="w-[95vw] sm:w-full max-w-2xl max-h-[80vh] bg-white dark:bg-[#0f172a]/95 backdrop-blur-xl border-gray-200 dark:border-white/10 text-gray-900 dark:text-white overflow-hidden flex flex-col shadow-2xl">
            <DialogHeader>
...
export default function CreativeRoulette() {
  return (
    <DashboardLayout>
      <CreativeRouletteInner />
    </DashboardLayout>
  );
}
